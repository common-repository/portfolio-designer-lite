 ...
 */
class portdesign_lessc extends \stdClass {
	...
	protected function injectVariables( $args ) {
		$this->pushEnv();
		$parser = new portdesign_lessc_parser( $this, __METHOD__ );
	...
	protected function makeParser( $name ) {
		$parser                = new portdesign_lessc_parser( $this, $name );
	...
	protected function newFormatter() {
		$className = 'portdesign_lessc_formatter_lessjs';
		if ( ! empty( $this->formatterName ) ) {
			if ( ! is_string( $this->formatterName ) ) {
				return $this->formatterName;
			}
			$className = "portdesign_lessc_formatter_$this->formatterName";
		}
...
// syntax tree
class portdesign_lessc_parser extends \stdClass {
	...
	public function __construct( $lessc, $sourceName = null ) {
		...
		if ( ! self::$operatorString ) {
			self::$operatorString = '(' . implode( '|', array_map( array( 'portdesign_lessc', 'preg_quote' ), array_keys( self::$precedence ) ) ) . ')';

			$commentSingle     = portdesign_lessc::preg_quote( self::$commentSingle );
			$commentMultiLeft  = portdesign_lessc::preg_quote( self::$commentMultiLeft );
			$commentMultiRight = portdesign_lessc::preg_quote( self::$commentMultiRight );
	...
	protected function isDirective( $dirname, $directives ) {
		...
		$pattern = implode( '|', array_map( array( 'portdesign_lessc', 'preg_quote' ), $directives ) );
	...
	protected function expressionList( &$exps ) {
		...
		$exps = portdesign_lessc::compressList( $values, ' ' );
		return true;
	}
	...
	public function propertyValue( &$value, $keyName = null ) {
		...
		$value = portdesign_lessc::compressList( $values, ', ' );
		return true;
	}
	...
	protected function openString( $end, &$out, $nestingOpen = null, $rejectStrs = null ) {
		...
		$stop = array( "'", '"', '@{', $end );
		$stop = array_map( array( 'portdesign_lessc', 'preg_quote' ), $stop );
	...
	protected function string( &$out ) {
		...
		$patt = '([^\n]*?)(@\{|\\\\|' .
				portdesign_lessc::preg_quote( $delim ) . ')';
	...
	protected function literal( $what, $eatWhitespace = null ) {
		...
		if ( ! isset( self::$literalCache[ $what ] ) ) {
			self::$literalCache[ $what ] = portdesign_lessc::preg_quote( $what );
		}
	...
	protected function to( $what, &$out, $until = false, $allowNewline = false ) {
		...
		if ( ! $this->match( '(' . $validChars . '*?)' . portdesign_lessc::preg_quote( $what ), $m, ! $until ) ) {
			return false;
		}
		if ( $until ) {
...
class portdesign_lessc_formatter_classic extends \stdClass {
...
class portdesign_lessc_formatter_compressed extends portdesign_lessc_formatter_classic {
...
class portdesign_lessc_formatter_lessjs extends portdesign_lessc_formatter_classic {
