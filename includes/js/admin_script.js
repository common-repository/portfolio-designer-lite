...
jQuery(document).ready(function() {
    ...
    if (jQuery('#portfolio_custom_css').length) {
        ...
        var editor = wp.codeEditor.initialize(jQuery('#portfolio_custom_css'), editorSettings);
    }
    ...
    jQuery('.port-setting-handle > li').click(function(event) {
        if (jQuery(this).hasClass('clickDisable')) {
            ...
            jQuery.post(ajaxurl, {
                ...
                nonce: portdesigner_admin_translations.nonce,
            });
    ...
    if (jQuery('input[name=portfolio_enable_filter]:checked').val() == 1) {
        jQuery('#portfolio_enable_pagination').prop('checked', false);
        ...
        jQuery('.pdl_meta_title').hide();
        jQuery('.pdl_meta_table').hide();
        jQuery('.pdl_meta_typography').hide();
    }
    ...
    jQuery('input[name="portfolio_post_tilte_max"]').on('click', function() {
        if (jQuery(this).val() == '1') {
            jQuery('.layout_unique_post_tilte').show();
        } else {
            jQuery('.layout_unique_post_tilte').hide();
        }
    });
    
    if (jQuery('#portfolio_post_tilte_max_0').prop('checked') == true) { 
        jQuery('.layout_unique_post_tilte').hide();
    }
    ...
    jQuery('#portfolio_order_by').change(function() {
        if (jQuery(this).val() == 'rand') {
            jQuery('.portfolio_order_tr').hide();
        } else {
            jQuery('.portfolio_order_tr').show();
        }
    });
    if (jQuery('#portfolio_order_by').val() == 'rand') {
        jQuery('.portfolio_order_tr').hide();
    } else {
        jQuery('.portfolio_order_tr').show();
    }
    ...
    /* popup closed */
    jQuery('.ui-widget').on('click', '.ui-widget-overlay', function() {
    ...
    jQuery('.content_background_color_tr').show();
    ...
    if (jQuery('#portfolio_content_position').val() == 'overlay_image') {
        jQuery('.content_background_color_tr').hide();
    ...
    jQuery(document).on('change', '#portfolio_content_position', function() {
        if (jQuery(this).val() == 'overlay_image') {
            ...
            jQuery('.content_background_color_tr').fadeIn();
    ...
    jQuery('#portfolio_enable_overlay').change(function() {
        if (jQuery('#portfolio_enable_overlay').is(':checked')) {
            var $overlay = 'enable';
        } else {
            var $overlay = 'disable';
        }
        portfolioEnableOverlay($overlay);
    });
    ...
    if (jQuery('#portfolio_enable_overlay').prop('checked') == true) {
        ...
        jQuery('.portfolio_image_overlay').show();
        jQuery('.portfolio_popup_tr').show();
        jQuery('.pdl_button_title').show();
        jQuery('.pdl_button_table').show();
    } else {
        jQuery('.portfolio_overlay_tr').hide();
        jQuery('.portfolio_image_overlay').hide();
        jQuery('.portfolio_border_radius_tr').hide();
        jQuery('.portfolio_popup_tr').hide();
        jQuery('.pdl_button_title').hide();
        jQuery('.pdl_button_table').hide();
    }
    jQuery('#portfolio_enable_overlay').on('click', function() {
        if (jQuery(this).prop('checked') == true) {
            jQuery('.portfolio_overlay_tr').fadeIn();
            jQuery('.portfolio_image_overlay').fadeIn();
            jQuery('.portfolio_border_radius_tr').fadeIn();
            jQuery('.portfolio_popup_tr').fadeIn();
            jQuery('.pdl_button_title').show();
            jQuery('.pdl_button_table').show();
        } else {
            jQuery('.portfolio_overlay_tr').fadeOut();
            jQuery('.portfolio_image_overlay').fadeOut();
            jQuery('.portfolio_border_radius_tr').fadeOut();
            jQuery('.portfolio_popup_tr').fadeOut();
            jQuery('.pdl_button_title').hide();
            jQuery('.pdl_button_table').hide();
        }
    });
    ...
    jQuery('.pro-feature, .pro-feature ul, .pro-feature input, .pro-feature a, .pro-feature > .wp-picker-container, #portfolio-show-preview').on('click', function(e) {
        e.preventDefault();
...
function portfolioEnableOverlay($overlay) {
    jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
        data: {
            action: 'get_content_position',
            overlay: $overlay,
            nonce: portdesigner_admin_translations.nonce,
        },
        success: function(response) {
            jQuery('.content_position_tr').html(response);
            jQuery('.select-cover select').chosen();
...
function pdl_default_value() {
    ...
    jQuery('#portfolio_enable_overlay').prop('checked', false);
    jQuery('#portfolio_overlay_tr').show();
    jQuery('#portfolio_content_position').val('bottom_image');
    ...
    jQuery('#portfolio_title_font_color').iris('color', '#FFF');
    jQuery('#portfolio_title_font_size').val('14');
    jQuery('#portfolio_content_font_color').iris('color', '');
    jQuery('#portfolio_content_font_size').val('12');
    jQuery('#portfolio_meta_font_color').iris('color', '');
    jQuery('#portfolio_meta_font_size').val('12');
    jQuery('#portfolio_button_font_color').iris('color', '#000');
    jQuery('#portfolio_button_font_size').val('0');
    jQuery('#content_background_color').iris('color', '');
    jQuery('#portfolio_title_font_color').val('#FFF');
    jQuery('#portfolio_meta_font_color').val('#000000');
    jQuery('#portfolio_content_font_color').val('');
    jQuery('#portfolio_button_font_color').val('#000');
    jQuery('#content_background_color').val('');
    jQuery("#portfolio-image-action-button").removeClass("pdl-remove_image_button");
    jQuery("#portfolio-image-action-button").addClass("pdl-upload_image_button");
    jQuery("#portfolio-image-action-button").val("Upload Image");
    jQuery(".portfolio_default_image_holder > img").slideDown().remove();
    jQuery("#portfolio_default_image_id").val("");
    jQuery("#portfolio_default_image_src").val("");
...
jQuery('#portfolio_enable_social_share_settings').on("click", function() {
    if( 1 == jQuery('#portfolio_enable_social_share_settings:checked').val() ) {
...
if (jQuery('#portfolio_enable_social_share_settings').is(':checked')) {
    jQuery('#portfolio_enable_social_share_settings').closest('#pdlsocial').find('tr').removeClass('hide');
} else {
    jQuery('#portfolio_enable_social_share_settings').closest('#pdlsocial').find('tr').addClass('hide');
    jQuery('.keep-it-on').removeClass("hide");
...
if (jQuery('#portfolio_button_type_1').is(':checked')) {
...
function pdl_submit_optin(options) {
    ...
    result.type = options;
    result.nonce = portdesigner_admin_translations.nonce;
...
jQuery(document).ready(function($) {
    $(document).on('change', 'input[name^="display_taxonomy_"]', function() { 
        $(this).parents('div.port-typo-options').find('label.desc_checkbox').hide();
        if($(this).val() == 1) $(this).parents('div.port-typo-options').find('label.desc_checkbox').show();
    });
    $('input[name^="display_taxonomy_"]:checked').change();
});